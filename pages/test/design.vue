...
export default {
  methods: {
    ...
    testFun23() {
      ...
      const arr = new Array('xxx', 'yyy', 'zzz');
      arr.selfMap((item, index) => {
    ...
    testFun21() {
      ...
      // 测试
      const feiji1 = new feiji('feiji1');
      const feiji2 = new feiji('feiji2');
      tatai.zhuce(feiji1);
      tatai.zhuce(feiji2);
    ...
    testFun19() {
      ...
      const CanvasCommand = (function () {
        ...
        const ctx = canvas.getContext('2d'); // 在命令模式内部封装了唯一性静态对象，外部只是调用命令，避免了意外修改
        const Action = { // 枚举具体命令
          // 填充色彩
          fillStyle(c) {
            ctx.fillStyle = c;
          },
          moveTo(x, y) { // 参数虽然不同，但可以统一调用
            ctx.moveTo(x, y);
          },
        };
    ...
    testFun18() {
      ...
      3);
      // 中间层
      const dialog = new Handler(app, 1);
      dialog.run = function () {
        console.log('中间层执行前...');
        // 这里做具体的处理操作
      ...
      // 最内层
      const button = new Handler(dialog, 2);
      button.run = function () {
        console.log('最内层执行前...');
        // 这里做具体的处理操作
    ...
    testFun17() {
      ...
      InputStrategy.addStrategy('phone', (value) => (/^\d{3}\-\d{8}$|^\d{4}\-\d{7}$/.test(value) ? '' : '请输入正确的电话号码格式!'));
      // 验证
    ...
    testFun4() {
      ...
      const Person = function (name, work) {
        const _person = new Human(); // 创建应聘者缓存对象
        ...
        // 实例化
      const person = new Person('xiaoming', 'code');
    },

<template>
<uni-shadow-root class="vant-weapp-grid-item-index"><view :class="'custom-class '+(utils.bem('grid-item', { square }))" :style="viewStyle" @click="onClick">
  <view :class="'content-class '+(utils.bem('grid-item__content', [direction, { center, square, clickable, surround: border && gutter }]))+' '+(border ? 'van-hairline--surround' : '')" :style="contentStyle">
    ...
    <block v-else>
      <view class="van-grid-item__icon icon-class">
        <van-icon v-if="icon" :name="icon" :dot="dot" :info="badge || info" :size="iconSize"></van-icon>
